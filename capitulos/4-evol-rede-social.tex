...
O Noosfero foi desenvolvido na linguagem \textit{Ruby}, na verão 2.2.0, pois possui uma sintaxe simples, que facilita a manutenibilidade do sistema, característica importante em projetos de software livre que tendem a atrair colaboradores externos \cite{meirelles2013}. Também utiliza o \textit{framework} aplicações web \textit{Ruby on Rails} \footnote{Disponível em: \url{http://rubyonrails.org/}}, versão 4. E utiliza também padrões arquiteturais de software Model-View-Controller (MVC), e o padrão de plugins que serão apresentados na seção \ref{arquitetura}. Além disso , por questões de segurança, o Noosfero é homologado para a versão \textit{stable} do \textit{Debian}, acompanhando a versão do \textit{Ruby} e do \textit{Rails} dessa distro.
...
\begin{figure}[h]
    ...
    \caption[Relações entre entidades de domínio ambiente, domínio e perfis]{Relações entre entidades de domínio ambiente, domínio e perfis. Extraído de: \cite{bucher2013rede}}
    \label{domain_main}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption[Entidades de domínio: tipos de perfis]{Entidades de domínio: tipos de perfis. Extraído de: \cite{bucher2013rede}}
...
\begin{figure}[h]
    ...
    \caption[Entidades de domínio: tipos de artigos]{Entidades de domínio: tipos de artigos. Extraído de: \cite{bucher2013rede}}
